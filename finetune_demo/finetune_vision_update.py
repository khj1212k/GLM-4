...
# 이미지 전처리 함수 (GLM-4V Vision 설정에 맞춰야 함)
...
def preprocess_for_eval(batch: dict[str, Any]) -> dict[str, Any]:
    image_path = batch.get("image")
    for conv in batch["messages"]:
        if isinstance(image_path, str):
            conv["image"] = image_to_tensor(image_path)
    return batch

    
# 파일 상단에 캐시 디렉터리 상수 추가
MAPPED_ROOT = Path("/workspace/mapped_data")
...
class DataManager(object):
    ...
    def get_dataset(
        ...
        # 1) 캐시 경로 계산 (e.g. /workspace/mapped_data/train)
        cached_dir = MAPPED_ROOT / str(split)  # str(split) --> "train", "validation", "test"
        if cached_dir.exists():
            print(f"[INFO] ✅ cached dataset found → {cached_dir}")
            return load_from_disk(str(cached_dir))

        # 2) 원본 로드
        orig_dataset = self._get_dataset(split)
        if orig_dataset is None:
            print(f"[WARN] ⚠️  split '{split}' not found")
            return None

        # 3) 맵핑
        remove_cols = orig_dataset.column_names if remove_orig_columns else None
        mapped_ds = orig_dataset.map(
            process_fn,
            batched=batched,
            ...
            batch_size=500,
            desc=f"[{str(split)}] Mapping processed inputs",
        )

        # 4) 캐시에 저장
        mapped_ds.save_to_disk(str(cached_dir))
        print(f"[INFO] 💾 saved mapped dataset → {cached_dir.resolve()}")
...
def process_batch_eval(
    ...
    for conv in batched_conv:
        # 수정: 텐서로 변환하지 않고 원래 이미지 경로 혹은 PIL 객체만 유지
        if isinstance(conv[0].get("image"), str):
            conv[0]["image"] = Image.open(conv[0]["image"]).convert("RGB")
        elif isinstance(conv[0].get("image"), Image.Image):
            pass  # 그대로 유지
        else:
            conv[0]["image"] = img
        ...
        for end_idx in range(1, len(dialogue_parts)):
            ...
            output_segment    = input_ids[dialogue_parts[end_idx]: user_idx[end_idx]] + [151336]
            ...
            # ── 핵심: 이미지(및 BOI/EOI)는 첫 세그먼트에만 ─────────────
            if end_idx == 1:                               # 첫 번째 조각
        ...
        for checkpoint_str in dirlist:
            if "checkpoint-" in checkpoint_str and "tmp" not in checkpoint_str:
                try:
                    ...
                    continue  # 숫자가 아닌 경우 무시 (.ipynb_checkpoints 같은 것)
